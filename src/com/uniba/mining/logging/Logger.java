...
import com.uniba.mining.utils.StringPlaceholders;
import com.uniba.mining.utils.StringPlaceholders.Placeholder;
...
public class Logger {
	...
	private static void addExtraAttributes(XAttributeMap attributes, IModelElement modelElement) {
		if (modelElement instanceof IHasChildrenBaseModelElement) {
			addAttribute(attributes, LogAttribute.UML_ELEMENT_CHILDREN, Arrays.toString(
					Arrays.stream(modelElement.toChildArray()).map(IModelElement::getId).toArray(String[]::new)));
		}
		if (modelElement instanceof IOperation) {
			addAttribute(attributes, LogAttribute.PARAMETERS,
					Arrays.toString(Arrays.stream(((IOperation) modelElement).toParameterArray())
							.map(IModelElement::getId).toArray(String[]::new)));
		}
	...
	public static void createEvent(LogActivity logActivity, IProject project, String propertyName,
		...
		System.out.println(
				String.format("%s %s %s %s", logActivity.toString(), project.getName(), propertyName, propertyValue));
		long timestamp = Instant.now().toEpochMilli();
	...
	public static void createEvent(LogActivity logActivity, IDiagramUIModel diagramUIModel, String propertyName,
			String propertyValue) {
		System.out.println(String.format("%s %s %s %s", logActivity.toString(), diagramUIModel.getName(), propertyName,
				propertyValue));
		long timestamp = Instant.now().toEpochMilli();
		String activityId = xIdFactory.createId().toString();
	...
	public static void createEvent(LogActivity logActivity, IModelElement modelElement, ModelType sourceModelType,
		...
		System.out.println(String.format("%s %s %s %s %s", logActivity.toString(), modelElement.getName(),
				sourceModelType.getName(), propertyName, propertyValue));
		long timestamp = Instant.now().toEpochMilli();
		String activityId = xIdFactory.createId().toString();
	...
	public void info(String message) {
		viewManager.showMessage(String.format("%s: %s at %s", classId.getSimpleName(), message, LocalDateTime.now()),
				Config.PLUGIN_ID);
	}
	...
	public void info(String message, Object... args) {
		...
		viewManager.showMessage(
				String.format("%s: %s at %s", classId.getSimpleName(), formattedMessage, LocalDateTime.now()),
				Config.PLUGIN_ID);
	}
}
